...
注 2：指针不能直接计算，不能像 C/C++ 中一样，直接对指针作加减操作，达到偏移访问的目的。在 Go 中，如果对指针
*/
func demo1() {
	var p1 *int
	var p2 *string
...
func demo2() {
	...
	fmt.Println("a的指针：", &a)
	p = &a
	fmt.Println("a的指针：", p, &a)

	var pp **int
	pp = &p
	fmt.Println("p的指针：", pp, "a的指针：", p)
	**pp = 3
	fmt.Println("p的指针：", pp, "pp的指针指向的数据：", *pp, "p的指针：", p)
	fmt.Println("pp的指针指向的数据：", **pp, "p的指针指向的数据：", *p)
	fmt.Println("a的数据：", a, "a的指针：", &a)
}
